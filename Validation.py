...
def odds_ratio(Y_real, Y_pred, base_Y_pred):
    conf_matrix = get_confusion_matrix(Y_real, Y_pred)
    base_conf_matrix = get_confusion_matrix(Y_real, base_Y_pred)

    TP, FP, _, _ = conf_matrix[0][0], conf_matrix[0][1], conf_matrix[1][0], conf_matrix[1][1]
    bTP, bFP, _, _ = base_conf_matrix[0][0], base_conf_matrix[0][1], base_conf_matrix[1][0], base_conf_matrix[1][1]

    if bTP * FP > 0:
        return TP * bFP / (bTP * FP)
    else:
        return 0
